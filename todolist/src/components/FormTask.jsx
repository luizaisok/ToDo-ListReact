...
function FormTask({ url }) {
  ...
  const { criarTarefa, editarTarefa, buscarPorId } = useFetch(url);
  ...
  useEffect(() => {
    if (id) {
      (async () => {
        const tarefaCarregada = await buscarPorId(id);
        if (tarefaCarregada) {
          setTarefa(tarefaCarregada);
        } else {
    ...
    <div className="p-6 max-w-lg mx-auto">
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <input
            ...
            required
            className="w-full border rounded px-3 py-2 outline-none focus:shadow-md"/>
